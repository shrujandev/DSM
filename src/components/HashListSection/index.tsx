...
const HashListSection = ({
  isSyncInProgress,
  uploadedFileHashes,
  setSelectedHash,
}: Props) => {
  const [lastElementHighlighted, setLastElementHighlighted] = useState(false);
  const [prevUploadedFileHashesLength, setPrevUploadedFileHashesLength] =
    useState(uploadedFileHashes.length);
  ...
  return (
    <Container title="Your stored file hashes">
      <>
        {isSyncInProgress ? (
          <div className={styles["hash-list-section__row"]}>
              ...
              <div
                className={classNames(styles["hash-list-section__row"], {
                  [styles["hash-list-section__row--highlighted"]]:
                    index === uploadedFileHashes.length - 1 &&
                    lastElementHighlighted,
                })}
                key={hash}
                onClick={() => handleSearch(hash)}
              >
                <span className={styles["hash-list-section__hash-index"]}>
                  {index + 1}
                </span>
                <span className={styles["hash-list-section__hash"]}>
                  {hash}
                </span>
              </div>
            ))}
