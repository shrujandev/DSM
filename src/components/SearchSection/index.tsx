...
import {
  downloadIpfsFileAsync,
  getFileFromChainAsync,
  getIpfsFileUrlAsync,
} from "utils";
...
interface Props {
  ...
  selectedHash: string | undefined | null;
}

const SearchSection = ({ isLoading, setLoading, selectedHash }: Props) => {
  ...
  const onSearch = async () => {
    if (hashStorageContract && accounts && searchFileHash) {
      setLoading(true);
      const file = await getFileFromChainAsync(
        searchFileHash,
        hashStorageContract
      );
      setLoading(false);
  ...
  const search = async (selectedHash: string) => {
    if (hashStorageContract && accounts && selectedHash) {
      setLoading(true);
      const file = await getFileFromChainAsync(
        selectedHash,
        hashStorageContract
      ...
      <>
        <div className={styles["search-section__search-container"]}>
          ...
          <button
            ...
            disabled={
              isLoading || searchFileHash === searchedFileDetails?.fileHash
            }
            onClick={onSearch}
          >
            Search
        ...
        {searchedFileDetails && (
          <div className={styles["search-section__file-details"]}>
            <h2 className={styles["search-section__subtitle"]}>
              File info read form the blockchain:
            </h2>
            <label className={styles["search-section__label"]}>File hash</label>
            <div>{searchedFileDetails.fileHash}</div>
            <label className={styles["search-section__label"]}>
              IPFS hash (CID)
            </label>
        ...
        {isFileSearchError && (
          <div className={styles["search-section__error"]}>
            Failed to find the file. Please try again.
          </div>
        )}
      </>
    </Container>
